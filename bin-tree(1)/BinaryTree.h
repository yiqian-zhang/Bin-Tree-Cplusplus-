...
//  二叉树方法声明及重载编译系统默认方法声明:
    void CountLeaf(BinTreeNode<ElemType> *p,int &count);
	BinaryTree();						// 无参数的构造函数
...
// 二叉树类的实现部分
template <class ElemType>
void BinaryTree<ElemType>::CountLeaf(BinTreeNode<ElemType> *p,int &count){
    if(p!=NULL)
    {
               if((p->leftChild==NULL)&&(p->rightChild==NULL)) count++;
               CountLeaf(p->leftChild,count);
               CountLeaf(p->rightChild,count);
    }
}
