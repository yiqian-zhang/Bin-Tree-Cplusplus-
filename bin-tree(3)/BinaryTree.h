...
//  二叉树方法声明及重载编译系统默认方法声明:
    int CountBreadth(BinTreeNode<ElemType> *r);
	BinaryTree();						// 无参数的构造函数
...
// 二叉树类的实现部分
template <class ElemType>
int BinaryTree<ElemType>::CountBreadth(BinTreeNode<ElemType> *r)
{
        if (r == NULL)
        return 0;
        ...
        while (!q.IsEmpty()) {       // q非空
             ...
             CurLevelWidth=q.GetLength();
             maxWidth = CurLevelWidth > maxWidth ? CurLevelWidth :  maxWidth;
