...
//  二叉树方法声明及重载编译系统默认方法声明:
    void NonRecurringInOrder(void (*Visit)(const ElemType &)) const;
	BinaryTree();						// 无参数的构造函数
...
template <class ElemType>
void BinaryTree<ElemType>::NonRecurringInOrder(void (*Visit)(const ElemType &)) const
//无递归地中序遍历以r为根的二叉树
{
    LinkStack<BinTreeNode<ElemType> *> q;	// 定义栈q
